...
def parse_date(x):
	return date2num(DT.datetime.strptime(x, "%m/%d/%Y %H:%M"))
...
def make_stats_plot(filename_in, plotTitle, startTime=0, endTime=0, forecastTime=0):
	...
	pet = data[:,3]
	rain = precip #- pet
	times2 = [date2num(DT.datetime.strptime(s, "%m/%d/%Y %H:%M")) for (s) in times]
	if startTime == 0:
        	startInd = 0
        	numTimes = len(times2)
        	endInd = numTimes
	...
	times_obs2 = [date2num(DT.datetime.strptime(s, "%m/%d/%Y %H:%M")) for (s) in times_obs]
	...
	simp, = ax.plot(ind, sim[startInd:endInd], linewidth=1, color='blue', alpha=0.75)
	...
	precipp = ax2.plot(ind, rain[startInd:endInd], linewidth=1, color='green', alpha=0.75)
	if forecastTime > 0:
		forecastX = 0
		while times2[forecastX] <= forecastTime:
			forecastX = forecastX + 1
		forecastX = forecastX - startInd
		ax.axvline(x=forecastX, linewidth=1, color='burlywood')
	...
	fig.text(0.01, 0.01, "EF5-Stats: %s" % filename_in, size=10)
	plt.title(plotTitle)
	plt.show()
...
if __name__ == '__main__':
	...
	make_stats_plot(sys.argv[1], outname, startTime, endTime,title)	
